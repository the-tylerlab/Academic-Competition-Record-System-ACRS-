    ...
    <div className="flex flex-col gap-6 animate-fade-in text-slate-900">
      <div className="bg-white p-6 md:p-8 rounded-xl border border-slate-200 shadow-2xs">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-5 border-b border-slate-100">
          <div>
            <h3 className="text-xl font-bold text-slate-900">ทำเนียบรายชื่อฐานข้อมูล</h3>
            <p className="text-base text-slate-500 mt-1 font-medium">จัดการข้อมูลนักเรียนที่มีในระบบ (นำเข้าจากฝ่ายทะเบียน)</p>
          </div>
          <div className="flex gap-2">
            <button 
              onClick={handleClearAll}
              className="px-4 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded-lg text-base font-bold transition-colors cursor-pointer flex items-center gap-2"
            >
            ...
            <button 
              ...
              className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-base font-bold transition-colors cursor-pointer shadow-sm flex items-center gap-2"
            >
            ...
            <button 
              ...
              className="px-5 py-2.5 bg-slate-900 hover:bg-slate-900/90 text-white rounded-lg text-base font-bold transition-colors cursor-pointer shadow-sm flex items-center gap-2"
            >
          ...
          <div className="flex-1">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-lg text-base focus-visible:ring-2 focus-visible:ring-slate-900 focus-visible:outline-hidden transition shadow-3xs outline-none bg-slate-50 focus-visible:bg-white font-medium text-slate-900"
              />
            </div>
          </div>
          <select
            ...
            className="px-4 py-2.5 border border-slate-200 rounded-lg text-base font-bold text-slate-700 bg-white focus-visible:ring-2 focus-visible:ring-slate-900 focus-visible:outline-hidden shadow-3xs cursor-pointer"
          >
        ...
        <div className="border border-slate-200 rounded-xl overflow-x-auto shadow-2xs">
              ...
              {filteredStudents.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-400">
                    <p className="font-bold text-base">ไม่พบข้อมูลนักเรียน</p>
                    <p className="text-sm mt-1">ลองเปลี่ยนคำค้นหา หรือนำเข้าข้อมูลใหม่</p>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={i} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button 
                        ...
                        className="text-slate-400 hover:text-indigo-600 transition-colors p-1.5 font-bold text-sm uppercase mr-2 tracking-wider cursor-pointer"
                      >
                        แก้ไข
                      </button>
                      <button 
                        onClick={() => handleDelete(st.studentId, st.id)}
                        className="text-slate-400 hover:text-rose-600 transition-colors p-1.5 font-bold text-sm uppercase tracking-wider cursor-pointer"
                      >
                        ลบ
                      </button>
                    </td>
          ...
          <div className="bg-slate-50 border-t border-slate-200 px-6 py-3 flex justify-between items-center text-sm font-bold text-slate-500">
      ...
      {showFormModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden transform transition-all animate-modal-in">
            ...
            <form onSubmit={handleSaveStudent} className="p-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-slate-500 uppercase tracking-wider mb-1.5">รหัสประจำตัว</label>
                  <input type="text" required value={formData.studentId} onChange={(e) => setFormData({...formData, studentId: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-base focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="เช่น 10001" disabled={isEditing} />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-500 uppercase tracking-wider mb-1.5">ชื่อ-นามสกุล</label>
                  <input type="text" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-base focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="เช่น นายสมชาย ใจดี" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-slate-500 uppercase tracking-wider mb-1.5">ระดับชั้น</label>
                    <input type="text" required value={formData.grade} onChange={(e) => setFormData({...formData, grade: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-base focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="เช่น ม.4" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-500 uppercase tracking-wider mb-1.5">ห้อง</label>
                    <input type="text" required value={formData.room} onChange={(e) => setFormData({...formData, room: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-base focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="เช่น 1" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-slate-500 uppercase tracking-wider mb-1.5">แผนการเรียน</label>
                    <select value={formData.program} onChange={(e) => setFormData({...formData, program: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-base focus:ring-2 focus:ring-indigo-500 outline-none bg-white">
                      <option value="Normal">Normal</option>
                      <option value="EP">EP</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-500 uppercase tracking-wider mb-1.5">อีเมล</label>
                    <input type="email" required value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-base focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="email@school.ac.th" />
                  </div>
                </div>
              </div>
              
              <div className="mt-8 flex justify-end gap-3">
                <button type="button" onClick={() => setShowFormModal(false)} className="px-4 py-2 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg text-base font-bold transition-colors cursor-pointer">
                  ยกเลิก
                </button>
                <button type="submit" disabled={isSaving} className="px-5 py-2 bg-slate-900 hover:bg-slate-900/90 text-white rounded-lg text-base font-bold transition-colors disabled:opacity-50 cursor-pointer shadow-sm">
